...
- Pixels are written in the warm ramp the program has always displayed - the scheme's channels recombined so bright points glow red, with a lifted tail and a soft shoulder that keeps the brightest points short of white - and the recombination is baked into the buffer, so the rendered frame, the browser and the saved PNG show the same colors.
